...
pub fn sync(args: SyncArgs) -> Result<(), M2AnkiError> {
    ...
    let note_type = match client.model.find_by_name(&model_name) {
        Ok(model) => model,
        Err(_) => {
    ...
    // }
    //
    // let mut failed_notes: Vec<(PathBuf, Vec<(String, Note)>)> = Vec::new();
    ...
    //     let mut failed_in_deck: Vec<(String, Note)> =
