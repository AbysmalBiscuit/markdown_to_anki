use std::{collections::HashMap, sync::Arc};

use rayon::iter::{IntoParallelIterator, ParallelIterator};

use super::{error::APIError, http_client::HttpClient, response::Response};

pub mod model_response {
    use serde::{Deserialize, Serialize};
    #[derive(Debug, Serialize, Deserialize)]
    #[serde(rename_all = "camelCase")]
    pub struct Model {
        sortf: usize,
        did: usize,
        latex_pre: String,
        latex_post: String,
        #[serde(rename = "mod")]
        mod_: usize,
        usn: i64,
        vers: Vec<String>,
        #[serde(rename = "type")]
        type_: usize,
        css: String,
        name: String,
        flds: Vec<Field>,
        tmpls: Vec<Template>,
        tags: Vec<String>,
        pub id: u64,
        #[serde(skip_serializing)]
        req: Vec<String>,
    }

    #[derive(Debug, Serialize, Deserialize)]
    pub struct Field {
        name: String,
        ord: usize,
        sticky: bool,
        rtl: bool,
        font: String,
        size: usize,
        media: Vec<Media>,
    }

    #[derive(Debug, Serialize, Deserialize)]
    pub struct Media {
        source: String,
        filename: String,
        media_type: MediaType,
    }

    #[derive(Debug, Serialize, Deserialize)]
    pub enum MediaType {
        Audio,
        Image,
        Video,
    }

    #[derive(Debug, Serialize, Deserialize)]
    struct Template {
        name: String,
        ord: usize,
        qfmt: String,
        afmt: String,
        did: Option<String>,
        bqfmt: String,
        bafmt: String,
    }
}

pub mod create_model_params {

    use std::collections::HashMap;

    use serde::{Deserialize, Serialize};
    #[derive(Debug, Serialize)]
    #[serde(rename_all = "camelCase")]
    pub struct Params {
        model_name: String,
        in_order_fields: Vec<String>,
        css: String,
        is_cloze: bool,
        card_templates: Vec<HashMap<String, String>>,
    }
}

#[derive(Debug, Clone)]
pub struct ModelClient {
    http_client: Arc<HttpClient>,
}

impl ModelClient {
    pub fn new(client: Arc<HttpClient>) -> Self {
        ModelClient {
            http_client: client,
        }
    }

    pub fn update_model_styling(
        &self,
        model_name: &str,
        css: &str,
    ) -> Result<Response<Option<()>>, APIError> {
        self.http_client.request(
            "updateModelStyling",
            Some(update_model_styling_params::UpdateModelStylingParams::new(
                model_name, css,
            )),
        )
    }

    pub fn get_all_names(&self) -> Result<Vec<String>, APIError> {
        let response: Response<Vec<String>> = self.http_client.request("modelNames", None::<()>)?;
        Ok(response.result)
    }

    pub fn find_by_name(&self, model_name: &str) -> Result<String, APIError> {
        let models = self.get_all_names()?;
        models
            .into_par_iter()
            .find_first(|name| name.eq(model_name))
            .ok_or_else(|| APIError::ModelNotFound(model_name.into()))
    }

    pub fn create_model(
        &self,
        model_name: &str,
        in_order_fields: Vec<&str>,
        css: Option<&str>,
        is_cloze: bool,
        card_templates: Vec<HashMap<String, String>>,
    ) -> Result<model_response::Model, APIError> {
        todo!()
    }
}

pub mod update_model_styling_params {
    use serde::Serialize;

    #[derive(Debug, Serialize)]
    pub struct UpdateModelStylingParams {
        model: InnerParams,
    }

    #[derive(Debug, Serialize)]
    struct InnerParams {
        name: String,
        css: String,
    }

    impl UpdateModelStylingParams {
        pub fn new(name: &str, css: &str) -> Self {
            UpdateModelStylingParams {
                model: InnerParams {
                    name: name.into(),
                    css: css.into(),
                },
            }
        }
    }
}
